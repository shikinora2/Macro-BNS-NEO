...
# Import các module giao diện (các trang)
from gui.status_overlay import StatusOverlay
from gui.home_tab import HomeTab
from gui.main_combo_tab import ComboTab
from gui.hp_tab import HPTab
from gui.mana_tab import ManaTab
from gui.skill_tab import SkillTab
from gui.crit_tab import CritTab
...
class AutoKeySenderApp:
    ...
    def _setup_ui(self):
        """
        Tạo giao diện người dùng chính với cấu trúc TabView được tối ưu hóa.
        """
        notebook = ctk.CTkTabview(self.root, corner_radius=8)
        notebook.pack(fill="both", expand=True, padx=5, pady=5)

        # SỬA LỖI: Ánh xạ tên tab tới lớp và tên thuộc tính cố định
        page_definitions = {
            "Trang Chủ": (HomeTab, "trang_chu_tab"),
            "Combo Chính": (ComboTab, "combo_chinh_tab"),
            "HP": (HPTab, "hp_tab"),
            "Mana": (ManaTab, "mana_tab"),
            "Skill": (SkillTab, "skill_tab"),
            "Crit": (CritTab, "crit_tab"),
            "Cài Đặt": (SettingsTab, "cai_dat_tab"),
            "Ứng Dụng Khác": (OtherAppsTab, "ung_dung_khac_tab"),
        }
        
        self.lock_frames = {}
        protected_pages = ["Combo Chính", "HP", "Mana", "Skill", "Crit"]

        for name, (PageClass, attr_name) in page_definitions.items():
            tab_frame = notebook.add(name)
            
            if name in ["Combo Chính", "Ứng Dụng Khác"]:
                page = PageClass(master=tab_frame)
            else:
                page = PageClass(master=tab_frame, app=self)
            
            page.pack(fill="both", expand=True)
            
            # Sử dụng tên thuộc tính cố định đã định nghĩa
            setattr(self, attr_name, page)
            
            if name in protected_pages:
                lock_frame = ctk.CTkFrame(page, fg_color=("gray90", "gray15"))
                ctk.CTkLabel(lock_frame, text="Vui lòng kích hoạt License Key để sử dụng tính năng này.", font=ctk.CTkFont(size=14)).pack(expand=True)
                self.lock_frames[name] = lock_frame

        self.toggle_feature_tabs_lock(True) 
        notebook.set("Trang Chủ")

    def toggle_feature_tabs_lock(self, locked):
        for name, frame in self.lock_frames.items():
            if locked:
    ...
    def activate_license(self):
        user_key = self.trang_chu_tab.license_key_entry.get()
        if not user_key:
        ...
        if status == "VALID":
            ...
            self.toggle_feature_tabs_lock(False)
            self.trang_chu_tab.update_expiration_date(expiry_date, "valid")
            self.trang_chu_tab._silent_autosave()
        elif status == "EXPIRED":
            self.is_license_valid = False
            self.toggle_feature_tabs_lock(True)
            self.trang_chu_tab.update_expiration_date(expiry_date, "expired")
            self.trang_chu_tab.log_message(f"Key đã hết hạn vào ngày: {expiry_date}")
            ...
            self.is_license_valid = False
            self.toggle_feature_tabs_lock(True)
            self.trang_chu_tab.update_expiration_date("Không hợp lệ", "invalid")
            self.trang_chu_tab.log_message("Key không hợp lệ hoặc không tồn tại.")
            messagebox.showerror("Key không hợp lệ", "License key bạn nhập không đúng hoặc không tồn tại.\Vui lòng kiểm tra lại.", parent=self.root)
    ...
    def _handle_mouse_click_safely(self, x, y, button, pressed):
        if not self.is_license_valid:
            if pressed: self.trang_chu_tab.log_message("Lỗi: Vui lòng kích hoạt License Key hợp lệ để sử dụng macro.")
            return
        hotkey_map = {"Chuột giữa": mouse.Button.middle, "Chuột cạnh dưới": mouse.Button.x1, "Chuột cạnh trên": mouse.Button.x2}
        selected_hotkey_name = self.trang_chu_tab.hotkey_combo.get()
        selected_mode = self.trang_chu_tab.hotkey_mode_combo.get()
        ...
        if selected_mode == "Giữ để chạy":
            if pressed:
                if not self.is_hotkey_pressed:
                    ...
                    self.trang_chu_tab.update_status("running") 
                ...
                if self.is_hotkey_pressed:
                    self.is_hotkey_pressed = False
                    self.trang_chu_tab.update_status("stopped")
    ...
    def _on_close(self):
        if self.cai_dat_tab.hide_to_tray_var.get() == "on":
            self._hide_to_tray()
        else: self._quit_app()
    ...
    def _hide_to_tray(self):
        ...
        try:
            ...
            self.trang_chu_tab.log_message(f"Lỗi hide to tray: {e}. Vui lòng cài đặt pystray.")
            self._quit_app()
    ...
    def reset_all_configurations(self):
        ...
        if not answer:
            self.trang_chu_tab.log_message("Đã hủy thao tác reset.")
            return

        self.trang_chu_tab.log_message("Bắt đầu reset toàn bộ cấu hình...")
        
        self.trang_chu_tab.reset_to_default()
        ...
        for filename in ["autosave_config.json", "autosave.path"]:
            try:
                if os.path.exists(filename):
                    os.remove(filename)
                    self.trang_chu_tab.log_message(f"Đã xóa file: {filename}")
            except Exception as e:
                self.trang_chu_tab.log_message(f"Lỗi khi xóa {filename}: {e}")
        
        self.trang_chu_tab.log_message("Reset hoàn tất. Vui lòng khởi động lại ứng dụng nếu cần.")
        messagebox.showinfo("Hoàn tất", "Đã reset toàn bộ cấu hình về mặc định.", parent=self.root)

    def _run_macro(self):
        target_title = self.trang_chu_tab.window_combo.get()
        game_window = gw.getWindowsWithTitle(target_title)
        if not game_window:
            self.root.after(0, self.trang_chu_tab.log_message, "Cảnh báo: Không tìm thấy cửa sổ game.")
            self.is_macro_manager_running = False
            return
        game_window = game_window[0]
        
        main_actions = self._process_config_to_actions(self.combo_chinh_tab.get_config())
        
        sub_combo_full_config = {
        ...
        if main_actions:
            self.root.after(0, self.trang_chu_tab.log_message, "Chạy macro với Combo Chính...")
            ...
            self.root.after(0, self.trang_chu_tab.log_message, f"Sử dụng Combo {last_used_combo_name}.")
            ...
            while self.is_hotkey_pressed:
                ...
                if current_time_fps - fps_start_time >= 1.0:
                    ...
                    self.root.after(0, self.trang_chu_tab.update_performance_display, fps)
            ...
            while self.is_hotkey_pressed:
                ...
                if sub_actions:
                    self.root.after(0, self.trang_chu_tab.log_message, f"Thực thi Combo Phụ: {sub_combo_name}.")
        ...
        self.root.after(0, self.trang_chu_tab.update_performance_display, 0)
        self.root.after(0, self.trang_chu_tab.log_message, "Macro đã dừng.")
        self.is_macro_manager_running = False
    ...
    def _auto_load_last_config(self):
        ...
        if config_path_to_load:
            try:
                self.trang_chu_tab.log_message(f"Tự động tải từ: {config_path_to_load}")
                self.trang_chu_tab.load_configuration(filepath=config_path_to_load)
            except Exception as e:
                self.trang_chu_tab.log_message(f"Lỗi khi auto-load: {e}")
    
    def toggle_status_overlay(self, startup=False):
        if self.cai_dat_tab.show_overlay_var.get() == "on":
            pos = self.cai_dat_tab.overlay_pos
            self.status_overlay.move_window(pos['x'], pos['y'])
            self.status_overlay.show()
            if not startup: self.trang_chu_tab.log_message("Overlay trạng thái đã Bật.")
        else:
            self.status_overlay.hide()
            if not startup: self.trang_chu_tab.log_message("Overlay trạng thái đã Tắt.")

    def position_status_overlay(self):
        self.trang_chu_tab.log_message("Chọn một điểm trên màn hình để đặt overlay...")
        picker = ScreenPicker(self.root)
        result = picker.pick_coordinate()
        if result:
            self.cai_dat_tab.overlay_pos = result
            self.status_overlay.move_window(result['x'], result['y'])
            self.trang_chu_tab.log_message(f"Đã di chuyển overlay đến: ({result['x']}, {result['y']})")
            self.trang_chu_tab._silent_autosave()

    def test_single_image_condition(self, panel_data):
        self.trang_chu_tab.log_message("Bắt đầu kiểm tra điều kiện hình ảnh (DXcam)...")
        ...
        if match_location:
            ...
            self.trang_chu_tab.log_message(message)
    ...
    def test_hp_detection(self):
        self.trang_chu_tab.log_message("Đang kiểm tra nhận diện HP...")
        hp_config = self.hp_tab.get_config()
    ...
    def test_current_mana(self):
        self.trang_chu_tab.log_message("Đang kiểm tra nhận diện Mana...")
        mana_config = self.mana_tab.get_config()
        current_mana = self.condition_handler.get_current_mana_level(mana_config)
        
        if current_mana is not None:
            message = f"Nhận diện thành công! Mức Mana hiện tại: {current_mana}"
            self.trang_chu_tab.log_message(message)
            messagebox.showinfo("Kiểm tra Mana", message, parent=self.root)
        else:
            message = "Nhận diện thất bại. Vui lòng kiểm tra lại cấu hình tọa độ và màu sắc."
            self.trang_chu_tab.log_message(message)
            messagebox.showerror("Kiểm tra Mana", message, parent=self.root)
